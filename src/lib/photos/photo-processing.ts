...
import { MAX_PHOTO_BYTES, MAX_PHOTO_PIXELS } from './photo-limits';
import type { PhotoExtension } from './photo-keys';
import { centredPhotoCrop, photoCropPixels, type PhotoCrop } from './photo-crop';

const pngSignature = Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]);
const decodeOptions = { limitInputPixels: MAX_PHOTO_PIXELS, failOn: 'warning' as const };

function invalidPhoto(message: string, cause?: unknown): PhotoValidationError {
...
async function decodePhoto(bytes: Buffer) {
  ...
  try {
    ...
    if (cause instanceof PhotoValidationError) throw cause;
    throw invalidPhoto(
...
export async function processPhoto(bytes: Buffer, selectedCrop?: PhotoCrop) {
...
export async function processPhotoThumbnail(bytes: Buffer, crop: PhotoCrop) {
  const { pixels, raw } = await decodePhoto(bytes);
  return thumbnailPhoto(pixels, raw, crop);
}

export async function processPhotoPreview(bytes: Buffer) {
  const { pixels, raw } = await decodePhoto(bytes);
  return { image: await displayPhoto(pixels, raw), width: raw.width, height: raw.height };
}

export async function readPhotoDimensions(bytes: Buffer) {
