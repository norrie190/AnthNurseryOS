...
export function readR2Configuration(environment: Record<string, string | undefined> = process.env) {
  ...
  if (!parsed.success) {
    ...
    throw new Error(
      `Configure Plant photo storage in your private environment: ${fields.join(', ')}.`,
    );
  }
  return {
...
let storage: PlantPhotoStorage | undefined;

export function getPlantPhotoStorage(): PlantPhotoStorage {
  // Even an accidentally unmocked test cannot reach a configured real account.
  if (process.env.NODE_ENV === 'test' || process.env.VITEST) {
    throw new Error(
      'Real R2 access is disabled in automated tests. Use the Plant photo storage fake.',
    );
  }
  ...
  storage = {
    bucket,
    async upload(object) {
      assertPhotoObjectKey(object.key);
