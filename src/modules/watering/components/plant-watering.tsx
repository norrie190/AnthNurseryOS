...
function dueLabel(due: PlantWateringDetail['due']) {
  switch (due.status) {
    case 'NOT_CONFIGURED':
      return 'No watering schedule configured';
    case 'NEEDS_FIRST_WATERING':
      return 'Schedule configured — first watering not recorded';
    case 'OVERDUE':
      return `Overdue by ${Math.abs(due.daysUntilDue!)} ${Math.abs(due.daysUntilDue!) === 1 ? 'day' : 'days'}`;
    case 'DUE_TODAY':
      return 'Due today';
    case 'DUE_SOON':
  ...
  return (
    <section className={styles.section} aria-labelledby="watering-heading">
      <div className={styles.sectionHeader}>
        ...
        <p className={styles.status} data-status={due.status}>
          {dueLabel(due)}
        </p>
      </div>
