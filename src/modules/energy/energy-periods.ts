import { EnergyError } from './energy-errors';
import { calendarDateSchema } from '../../lib/calendar-date';
import { parseEnergy } from './energy-input';

export type EnergyInterval = { id: string; effectiveFrom: string; effectiveTo: string | null };
export function includesDate(period: EnergyInterval, date: string): boolean {
  return period.effectiveFrom <= date && (period.effectiveTo === null || period.effectiveTo > date);
}
export function overlaps(a: EnergyInterval, b: EnergyInterval): boolean {
  return (
    (a.effectiveTo === null || b.effectiveFrom < a.effectiveTo) &&
    (b.effectiveTo === null || a.effectiveFrom < b.effectiveTo)
  );
}
export function validateTimeline(periods: readonly EnergyInterval[]): void {
  const ordered = [...periods].sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom));
  for (const [index, period] of ordered.entries()) {
    parseEnergy(calendarDateSchema, period.effectiveFrom);
    if (period.effectiveTo !== null) parseEnergy(calendarDateSchema, period.effectiveTo);
    if (period.effectiveTo !== null && period.effectiveTo <= period.effectiveFrom)
      throw new EnergyError('VALIDATION_FAILED', 'The period end must be after its start.');
    if (index && overlaps(ordered[index - 1], period))
      throw new EnergyError(
        'OVERLAP',
        'These dates overlap another effective energy period. Review the history.',
      );
  }
}
export function planSettingChange<T extends EnergyInterval>(periods: readonly T[], from: string) {
  if (periods.some((period) => period.effectiveFrom === from))
    throw new EnergyError(
      'CONFLICT',
      'A period already starts on this date. Use a correction to change that entry.',
    );
  const previous = periods.find((period) => includesDate(period, from));
  const next = [...periods]
    .filter((period) => period.effectiveFrom > from)
    .sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom))[0];
  // A known gap after the covering period is not filled implicitly.
  const end = previous?.effectiveTo ?? next?.effectiveFrom ?? null;
  return { previous, effectiveTo: end };
}

type AdjacentAdjustment = { periodId: string; effectiveFrom?: string; effectiveTo?: string };
export function planCorrection<T extends EnergyInterval>(
  periods: readonly T[],
  current: T,
  proposed: T,
  adjustments: readonly AdjacentAdjustment[] = [],
): T[] {
  const changes = new Map<string, T>([[current.id, proposed]]);
  for (const adjustment of adjustments) {
    const adjacent = periods.find((period) => period.id === adjustment.periodId);
    if (!adjacent || changes.has(adjacent.id))
      throw new EnergyError('VALIDATION_FAILED', 'Choose each adjacent period once.');
    const left = adjacent.effectiveTo === current.effectiveFrom;
    const right = current.effectiveTo !== null && adjacent.effectiveFrom === current.effectiveTo;
    if (
      left &&
      adjustment.effectiveFrom === undefined &&
      adjustment.effectiveTo === proposed.effectiveFrom
    ) {
      changes.set(adjacent.id, { ...adjacent, effectiveTo: proposed.effectiveFrom });
    } else if (
      right &&
      adjustment.effectiveTo === undefined &&
      adjustment.effectiveFrom === proposed.effectiveTo
    ) {
      changes.set(adjacent.id, { ...adjacent, effectiveFrom: proposed.effectiveTo! });
    } else
      throw new EnergyError(
        'VALIDATION_FAILED',
        'An adjacent adjustment must explicitly match the shared corrected boundary.',
      );
  }
  // Breaking an existing shared boundary requires explicitly reviewing its neighbour.
  for (const adjacent of periods) {
    if (adjacent.id === current.id) continue;
    const brokenLeft =
      adjacent.effectiveTo === current.effectiveFrom &&
      proposed.effectiveFrom !== current.effectiveFrom;
    const brokenRight =
      current.effectiveTo !== null &&
      adjacent.effectiveFrom === current.effectiveTo &&
      proposed.effectiveTo !== current.effectiveTo;
    if ((brokenLeft || brokenRight) && !changes.has(adjacent.id))
      throw new EnergyError(
        'CONFLICT',
        'Review the adjacent period explicitly when changing this shared boundary.',
      );
  }
  validateTimeline(periods.map((period) => changes.get(period.id) ?? period));
  // Shrink/move whichever row is safe first, keeping immediate SQL exclusions valid.
  const state = new Map(periods.map((period) => [period.id, period]));
  const pending = [...changes.values()];
  const ordered: T[] = [];
  while (pending.length) {
    const index = pending.findIndex((period) =>
      [...state.values()].every((other) => other.id === period.id || !overlaps(period, other)),
    );
    if (index < 0)
      throw new EnergyError(
        'CONFLICT',
        'These boundary changes cannot be applied safely together.',
      );
    const [period] = pending.splice(index, 1);
    ordered.push(period);
    state.set(period.id, period);
  }
  return ordered;
}
