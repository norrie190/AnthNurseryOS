...
The Watering event, schedule, due-state and Plant-detail history workflows are complete. The nursery-wide Watering queue read model and its read-only `/watering` operational page are complete. Batch recording, Location filtering, notifications, generic Care integration and Dashboard Care integration remain pending.
