...
Zod is there for checking data at the edges of the app. Vitest and Testing Library are used for tests. The interface uses a small project specific design system made from global CSS variables, focused shared components and feature owned CSS Modules. This keeps the nursery visually consistent without adding a styling framework or turning presentation code into a separate product.

## Application shell

The main layout is shared by every route. Larger screens use a compact botanical sidebar and a small global Add Plant action. Smaller screens use a persistent bottom dock for Home, Plants, Add and Watering, with a focused More drawer for the remaining destinations. The navigation is the only client side part of the shell because it needs the current route and menu state. The rest stays server rendered by default.
...
Only working daily destinations are shown in the main navigation: Dashboard, Plants, Watering, Breeding, Equipment and Energy. Care and Expenses routes remain available during development but are not presented as primary destinations while they are placeholders. Later features should be added when their phase starts rather than appearing as empty promises.

The collection experience is photo led. Plant lists use responsive visual cards, while detail pages use a larger profile style identity header. Add Plant deliberately asks for name, status and Location first; parentage, purchase and notes remain available in one disclosure and can be completed later. The existing creation service, validation, immutable ANT reference and transaction rules remain authoritative. This is a presentation change and does not introduce draft records or schema changes.
...
Tariffs use the stable transaction advisory lock namespace 0x414e5448, key 1. Their SHA-256 timeline token covers sorted IDs/timestamps and retained void markers so an old empty token cannot become valid again after add/void. Repeatable Read report transactions combine bulk Equipment/period queries with the tariff snapshot. No schema, generic repository, singleton version record or cache is added.

The [energy browser workflow](energy-browser-flow.md) adds an Equipment detail section and tariff maintenance at `/energy/tariffs`. `/energy` is the main read only workspace. It shows configuration health, current projections, the applicable or next scheduled tariff, and one concise row for each active Equipment item capable of power tracking. A small energy specific shared editor calls strict server actions, which map text and date inputs to existing services. Open forms retain their original stale token; shared boundary corrections require explicit adjacent change confirmation. Inclusive human end dates are translated to exclusive service boundaries. Exact calculations run before rendering and are presented as strings, with projections distinguished from calendar history and incomplete subtotals. Archive confirmation warns about ongoing settings without changing them. No persistence rules or dependencies change.

The refreshed Energy presentation leads with one 30 day setup projection and makes its coverage state part of the same visual, followed by the exact supporting measures and the Equipment responsible for them. Equipment appears in compact photo led configuration cards rather than a dense utility table. Tariff management uses a prominent current rate panel and a quieter dated timeline, while Equipment detail pages separate today’s configured estimate, calendar month coverage and editable history. The wording continues to distinguish configured projections from live readings and bills. This redesign changes no calculations, history rules, concurrency tokens or persistence.
...
The homepage is a request time Server Component that calls this boundary once and passes the result to a dashboard presentation component. It is arranged as a practical nursery home screen: today’s Watering and Energy state come first, recent Plants and Equipment come next, with investment and shortcuts kept as supporting information. The UI keeps currencies separate, labels incomplete amounts and projection coverage, and links into the full Energy workspace without adding mutations. Recent thumbnails use the existing domain safe delivery paths and neutral client side image fallback; the dashboard never sees storage keys or signs provider URLs. Responsive CSS collapses each section into a natural reading order without adding a chart or widget framework.
...
The refreshed Equipment interface follows the same photo led collection language as Plants without sharing domain components. Active and archived inventories use responsive image cards with the EQP reference, category, Location and an explicit energy tracking capability label. The detail page uses the full display derivative as a profile image and keeps inventory, energy and photo workflows in the existing local sections. Add Equipment is essentials first: name, category, power tracking choice and Location are immediately visible, while manufacturer details, purchase information and notes remain available in one disclosure. This is a presentation change only; Equipment validation, immutable references, stale edit protection and all service boundaries remain unchanged.
...
The nursery-wide Watering queue read model is consumed by `/watering` through a server-side App Router page. The page groups the model's already-sorted entries and uses its category counts and due-state fields without querying Prisma or recalculating dates in presentation components. The interface is action first: a compact daily focus summary leads into photo-led Plant cards, while batch controls appear only after selection and stay reachable above the mobile navigation. Entries link to the existing Plant detail workflow. Batch selection submits Plant UUIDs and optional shared notes once through the transactional `recordWateringBatch` server action; the service remains authoritative for lifecycle eligibility and all-or-nothing writes. Primary photos use the existing safe thumbnail route and shared neutral fallback. Location filtering, notifications and Dashboard Care integration remain pending.

## Plant-detail Breeding workflow

The Plant detail page composes a focused Breeding section from `getPlantBreedingDetail`, which loads the owning Plant's inflorescences with nested pollinations and seed batches plus the small Plant selector set in fixed batched reads. Presentation components do not query Prisma or storage. Breeding server actions only adapt form data and revalidate the Plant detail; existing services remain authoritative for lifecycle eligibility, provenance coherence, transitions, corrections, voids and stale tokens.

The UI keeps the provenance hierarchy visible as Plant → Inflorescence → Pollination → SeedBatch. It exposes only valid ordinary transitions, keeps historical maintenance explicit, and preserves descendants of voided records for inspection. Seed harvest remains available for already-HARVESTED attempts and carries the current PollinationAttempt timestamp token, while active seed batches visibly lock provenance-sensitive pollination correction. This checkpoint does not add schema, migration, promotion, photos or R2 access.

## Nursery Breeding overview read model

The nursery-wide Breeding overview is a read-only model in `src/modules/breeding/breeding-overview-queries.ts`. It uses one Repeatable Read transaction and a relation-aware query to load non-void Inflorescences, PollinationAttempts, SeedBatches and minimal Plant/location metadata, then derives status counts and a bounded ten-item attention list in memory. Attention ordering is explicit by workflow priority, oldest relevant nursery date and stable Plant/record identifiers. Biological failure and terminal outcomes remain distinct from voided records, and Plant lifecycle metadata is retained rather than filtering historical breeding work. The `/breeding` page consumes the model without additional detail queries, photos, R2 access or writes.

The overview presents this existing read model as a three-stage nursery pipeline: Inflorescence, Pollination and Seed batches. A single focus panel leads into the stage summaries and the bounded attention queue. The type icons and visual hierarchy are presentation only; attention ordering, workflow status meanings, historical visibility and Plant links remain controlled by the read model.
