...
The approved Plant Management schema, initial migration and separate reference sequence migration are committed. They contain only Plant, PlantParentage, PlantPurchase, PlantPhoto, Location, PlantStatus and the ANT sequence. The creation data layer, browser form, detail page, Plant list, Edit Plant and Archive/Restore are committed. The photo architecture is committed, and the storage and data layer is implemented for review. A new migration adds the approved primary photo index without changing Prisma fields or prior migrations.
...
Operation contracts, memory and timeout limits, testing and development bucket instructions are in [Plant photo data layer](plant-photo-data-layer.md). No real provider verification has been performed. There are no photo HTTP endpoints, browser controls or list changes yet; request streaming limits and origin/CSRF checks belong to the next browser boundary before it can accept uploads.
