...
The UI keeps the provenance hierarchy visible as Plant → Inflorescence → Pollination → SeedBatch. It exposes only valid ordinary transitions, keeps historical maintenance explicit, and preserves descendants of voided records for inspection. Seed harvest remains available for already-HARVESTED attempts and carries the current PollinationAttempt timestamp token, while active seed batches visibly lock provenance-sensitive pollination correction. This checkpoint does not add schema, migration, promotion, photos, R2 access or a nursery-wide Breeding read model.
