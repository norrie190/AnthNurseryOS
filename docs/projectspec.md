...
The committed [energy history schema](equipment-energy.md) stores EquipmentPowerPeriod and a single nursery ElectricityTariff timeline, with exact decimal watts, daily hours and GBP pence per kWh, calendar date intervals, overlap protection and retained voided records. The [energy data layer](energy-data-layer.md) now supports recording, changing, correcting and voiding periods, with stale protection, exact derived calculations and coverage reporting. Gaps remain unknown rather than silently becoming zero, and historical reports include archived Equipment. Energy forms and dashboard integration are not built yet. Maintenance, photos and future equipment upgrades remain separate later work.
